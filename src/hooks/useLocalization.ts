...
export function useAvailableLocales() {
  return useQuery<LocaleInfo[], Error>({
    ...
    queryFn: async () => {
      const res = await clientGet<{ locales: LocaleInfo[] }>(phase4Client, '/localization/locales');
      return res.locales;
    },
...
export function useLocaleInfo(locale: SupportedLocale) {
  return useQuery<LocaleInfo, Error>({
    ...
    queryFn: async () => {
      return await clientGet<LocaleInfo>(phase4Client, `/localization/locales/${locale}`);
    },
...
export function useTranslationBundle(locale: SupportedLocale, namespace: string = 'common') {
  return useQuery<TranslationBundle, Error>({
    ...
    queryFn: async () => {
      return await clientGet<TranslationBundle>(phase4Client, '/localization/translations', {
...
export function useLocalizedContent<T extends LocalizedContent>(
  ...
  return useQuery<T, Error>({
    ...
    queryFn: async () => {
      return await clientGet<T>(phase4Client, `/localization/content/${contentType}/${contentId}`, {
...
export function useLocalizationSettings() {
  return useQuery<LocalizationSettings, Error>({
    ...
    queryFn: async () => {
      return await clientGet<LocalizationSettings>(phase4Client, '/localization/settings');
    },
  });
}
...
export function useAutoDetectLocale() {
  ...
  const detectLocale = useCallback((): SupportedLocale => {
    ...
    if (typeof navigator !== 'undefined') {
      deviceLocale =
        navigator.language || (navigator as { userLanguage?: string }).userLanguage || null;
    }
    ...
    if (deviceLocale && availableLocales) {
      ...
      const languageMatch = availableLocales.find((l: LocaleInfo) =>

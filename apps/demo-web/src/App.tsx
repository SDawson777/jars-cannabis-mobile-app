...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try { setCategories(await getJson<Category[]>(`/home/categories`)); } catch {}
      try { setFeatured(await getJson<Featured[]>(`/home/featured`)); } catch {}
      try { setWays(await getJson<Way[]>(`/home/ways`)); } catch {}
    })();
  }, []);
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: 16 }}>
      ...
      <section style={{ marginTop: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12 }}>
          {categories.map(c => (
            <div key={c.id} style={{ padding: 12, borderRadius: 12, background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,.05)' }}>
      ...
      <section style={{ marginTop: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
          {featured.map(f => (
            <div key={f.id} style={{ padding: 12, borderRadius: 12, background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,.05)' }}>
              <img src={f.image} alt={f.name} style={{ width: '100%', borderRadius: 8, aspectRatio: '1 / 1', objectFit: 'cover' }} />
      ...
      <section style={{ marginTop: 24, marginBottom: 48 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {ways.map(w => (
            <span key={w.id} style={{ padding: '8px 12px', borderRadius: 999, background: '#e5e7eb', color: '#111827' }}>{w.label}</span>
          ))}
        </div>
      </section>
    </div>

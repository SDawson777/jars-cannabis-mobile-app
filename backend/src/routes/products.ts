...
productsRouter.post('/products/:id/reviews', requireAuth, async (req, res) => {
	...
	const r = await (prisma as any).review.create({ data: { productId: req.params.id, userId: uid, rating, text: comment || text, recommend } });
	res.status(201).json({ review: r });
});
	// get product by slug
	productsRouter.get('/products/slug/:slug', async (req, res) => {
		const slug = req.params.slug;
		const p = await ((prisma as any).product.findManyBySlug ? (prisma as any).product.findManyBySlug({ where: { slug } }) : null);
	...
	productsRouter.get('/products/:id', async (req, res) => {
		...
		const related = (prisma as any).product.findManyByCategory ? await (prisma as any).product.findManyByCategory({ where: { category: p.category } }) : [];

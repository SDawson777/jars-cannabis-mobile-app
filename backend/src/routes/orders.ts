...
ordersRouter.post('/orders', requireAuth, async (req, res) => {
	...
	if (deliveryMethod === 'delivery') {
		...
		if (!addr.city || !addr.state || !addr.zipCode) return res.status(400).json({ error: 'invalid address' });
	}
	...
	const cart = preCart || await prisma.cart.findFirst({ where: { userId: uid }, include: { items: true } });
...
ordersRouter.get('/orders/:id/tracking', requireAuth, async (req, res) => {
	...
	res.json({ tracking: { status: (o.status || 'unknown').toLowerCase(), estimatedDelivery: new Date(Date.now() + 3600000).toISOString(), updates: [] } });
});

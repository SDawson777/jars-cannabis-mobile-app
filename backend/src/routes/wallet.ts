...
walletRouter.get('/wallet/balance', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const giftCardBalance = 25.0;
    const storeCredit = 5.0;
...
walletRouter.post('/checkout/apply-wallet', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (usePoints) appliedAmount += 5.0; // Example: 500 points = $5
    if (useGiftCards) appliedAmount += 25.0;
    if (useStoreCredit) appliedAmount += 5.0;

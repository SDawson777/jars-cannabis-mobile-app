...
profileRouter.get('/profile', requireAuth, async (req, res) => {
	...
	return res.json({ id: user.id, email: user.email, name: (user as any).name ?? null, phone: (user as any).phone ?? null, createdAt: user.createdAt });
});
...
profileRouter.put('/profile', requireAuth, async (req, res) => {
	...
	try {
		const updated = await prisma.user.update({ where: { id: uid }, data: { ...(name !== undefined ? { name } : {}), ...(phone !== undefined ? { phone } : {}), ...(email !== undefined ? { email } : {}) } });
		return res.json({ id: updated.id, email: updated.email, name: (updated as any).name ?? null, phone: (updated as any).phone ?? null, createdAt: updated.createdAt });
	} catch (err) {

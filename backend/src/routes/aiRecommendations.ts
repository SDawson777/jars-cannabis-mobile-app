      ...
      recommendations: [
        {
          ...
          reasoning: [
            { type: 'preference', text: 'Matches your preference for creative/energetic effects', weight: 0.4 },
            { type: 'journal', text: 'Similar to strains you rated highly in journal', weight: 0.35 },
        ...
        },
        {
          ...
          reasoning: [
            ...
            { type: 'journal', text: 'Your journal shows positive edible experiences', weight: 0.4 },
          ],
        },
      ],
      modelVersion: '2.3.1',
...
aiRecommendationsRouter.get('/ai/recommendations/similar/:productId', async (req: Request, res: Response) => {
  ...
  try {
    res.json({
      products: [
        { id: 'prod-2', name: 'Green Crack', similarity: 0.92, reason: 'Similar terpene profile' },
  ...
  try {
    res.json({
      recommendations: [
        ...
        { productId: 'prod-2', productName: 'Green Crack', matchScore: 0.88, effects: effectsArray },
      ],
    });
  } catch (error) {
  ...
  try {
    const moodProducts: Record<string, any[]> = {
      ...
      energized: [
        ...
        { productId: 'prod-cart-1', productName: 'Sativa Cart', matchScore: 0.90 },
      ],
      focused: [
    ...
    res.json({
      ...
      preferences: {
        categories: [
          ...
          { category: 'Edibles', weight: 0.30 },
          { category: 'Cartridges', weight: 0.25 },
        ],
        effects: [
          { effect: 'relaxed', weight: 0.35 },
          { effect: 'creative', weight: 0.25 },
          { effect: 'energetic', weight: 0.20 },
          { effect: 'sleepy', weight: 0.20 },
        ],
        terpenes: [
          { terpene: 'limonene', weight: 0.30 },
          { terpene: 'myrcene', weight: 0.25 },
          { terpene: 'caryophyllene', weight: 0.20 },
        ],
...
aiRecommendationsRouter.get('/ai/feedback', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    res.json({
      feedback: [
        { recommendationId: 'rec-1', rating: 5, helpful: true, purchased: true, createdAt: new Date().toISOString() },
        { recommendationId: 'rec-2', rating: 3, helpful: false, purchased: false, createdAt: new Date().toISOString() },
      ],
    });
  } catch (error) {
 ...
 */
aiRecommendationsRouter.post('/ai/feedback/rate', requireAuth, async (req: Request, res: Response) => {
...
aiRecommendationsRouter.get('/ai/trending', async (req: Request, res: Response) => {
  ...
  try {
    res.json({
      trending: [
        { productId: 'prod-1', productName: 'Blue Dream', rank: 1, trendScore: 0.95, salesGrowth: 45 },
        { productId: 'prod-2', productName: 'Wedding Cake', rank: 2, trendScore: 0.88, salesGrowth: 38 },

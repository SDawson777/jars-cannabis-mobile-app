...
| `.env`                              | Copy from `.env.example` and fill `SENTRY_DSN`, `EXPO_PUBLIC_API_BASE_URL`, all `EXPO_PUBLIC_FIREBASE_*` values, `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY`, `STRIPE_PUBLISHABLE_KEY`, `STRIPE_MERCHANT_ID`, and `EXPO_PUBLIC_OPENWEATHER_KEY`. |

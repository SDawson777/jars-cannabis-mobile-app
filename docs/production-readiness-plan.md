...
| **Build tooling**                  | `setup.sh`, `vercel-build.sh`, `scripts/*.ts`                                                           | `npm` (>=9), `node` (18.x/20.x), `husky`, `lint-staged`                                                                                                   | `setup.sh` runs `npm ci` root + backend, Firebase verification                             | Vercel runs `npm install --legacy-peer-deps` per `vercel.json`; ensure shrinkwraps committed.                         |
| **Backend API**                    | `backend/src/*` (schema in nimbus-cms repo)                                                             | `express@4/5`, `@prisma/client@6.16`, `firebase-admin@12`, `stripe@14`                                                                                    | `ts-node`, `tsc-alias`, `jest` backend config                                              | `express@5` pulled into root (Expo) but backend uses Express 4 – keep independence via `npm --prefix backend`.        |
